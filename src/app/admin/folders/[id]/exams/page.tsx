...
export default function ExamsAndQuestionPapersFolderPage() {
  ...
  const [folderSubView, setFolderSubView] = useState<'qp' | 'bank'>('qp');
  ...
  const [creationSource, setCreationSource] = useState<'bank' | 'blank'>('blank');
  ...
  const handleCreateExam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (creationSource === 'bank' && examForm.questionBankIds.length > 0) {
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <main className="flex-1 lg:pl-64 flex flex-col min-w-0">
        ...
        <div className="p-6 max-w-7xl w-full mx-auto space-y-6">
              ...
              {loading ? (
                ...
                <div className="glossy-card rounded-3xl p-12 text-center space-y-4 max-w-md mx-auto">
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    Create a custom exam paper or build one directly from the <strong>Question Bank</strong>.
                  </p>
                  <button
                    onClick={() => {
                      setCreationSource('blank');
          ...
          {/* ========================================================= */}
          {/* VIEW 2: QUESTION BANK REPOSITORY */}
          {/* ========================================================= */}
          {folderSubView === 'bank' && (
            <div className="space-y-6 animate-in fade-in">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-xl glossy-card rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 max-h-[90vh] flex flex-col">
            ...
            <div className="p-3 bg-slate-100 border-b border-slate-200/80 flex items-center justify-center space-x-2 shrink-0">
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-xs font-black flex items-center space-x-1.5 transition-all ${
                  creationSource === 'blank' ? 'bg-white text-indigo-700 shadow-2xs' : 'text-slate-600'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setFolderSubView('bank');
                  setIsNewExamOpen(false);
                }}
                className={`px-4 py-1.5 rounded-xl text-xs font-black flex items-center space-x-1.5 transition-all text-slate-600 hover:text-amber-700`}
              >
                <Database className="w-3.5 h-3.5 text-amber-600" />
            ...
            <form onSubmit={handleCreateExam} className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              {creationSource === 'bank' && examForm.questionBankIds.length > 0 && (
                <div className="p-3 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-800 text-xs font-bold flex items-center space-x-2">
                  <Database className="w-4 h-4 text-indigo-600 shrink-0" />
                  <span>{examForm.questionBankIds.length} questions from Question Bank will be imported.</span>
                </div>
              )}
