      ...
      <div className="lg:pl-64 flex flex-col min-h-screen min-w-0">
        <header className="bg-white border-b border-slate-200 py-4 px-6 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-base font-black text-slate-950 leading-tight">Candidate Landing Page Settings</h1>
              <p className="text-xs text-slate-500 font-medium">Configure all branding, campus options & labels dynamically</p>
            </div>
          </div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center space-x-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold py-2 px-3.5 rounded-xl border border-slate-200 transition-all shadow-xs"
          >
        ...
        <div className="p-6 md:p-8 space-y-6 max-w-5xl">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-2xs space-y-5">
                ...
                <form onSubmit={handleAddCampus} className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-blue-600 uppercase"
                  />
                  <button
                    type="submit"
                    disabled={isAddingCampus}
                    className="inline-flex items-center space-x-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs shadow-xs transition-all disabled:opacity-50 shrink-0"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pt-2">
                  {campuses.map((c) => (
                    <div
                      key={c.id}
                      className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs font-bold text-slate-900"
                    >
                ...
                <form onSubmit={handleSaveSettings} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-indigo-600"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-black text-slate-900 focus:outline-none focus:border-indigo-600"
                    />
                  </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      <div className="flex-1">
                        <input
                          ...
                          className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-semibold text-slate-900 focus:outline-none focus:border-indigo-600"
                        />
                        <p className="text-2xs text-slate-500 mt-1 font-medium">
                          Use local path (e.g. <code className="bg-slate-200 px-1 py-0.5 rounded text-slate-800">/logo.svg</code> or <code className="bg-slate-200 px-1 py-0.5 rounded text-slate-800">/logo.png</code>) or any public image URL.
                        </p>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-200 flex items-center justify-center min-w-[120px] shadow-xs">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center space-x-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-6 rounded-xl text-xs shadow-xs transition-all disabled:opacity-50"
                    >
